...
* 매수이유
  - 전고점을 돌파 했으며 7만원 넘게 까지 매물대도 거의 없는 상태 이다.
  - 이재명 정부의 주식시장 활성화

    ![삼성전자-전고점-돌파](image-4.png)

* 투자금액 - 100만원
  - 인버스 손절한 금액만 공부하기 위해서 매수 한다.
    + 돌파한날 KODEX CD 시간외 매도가 안되어 매수하지 못했는데 당일 KRX 매도 해야 하는데 9시 이후야 가능
  - 돌파 이후 대체로 10% 정도는 무난히 상승할것으로 예상되어 있는 상태. HBM4 개발등 이슈가 여러가지 있고. 매출이 바닥권일 가능성
    + 매수는 저점에서 해야 되기 때문에 매출이 가장 좋지 않을때가 매수 타점.
  - 손절이나 이익실현 모두 20일선을 5일선이 하향 돌파하는 데드크로스가 발생할때.
