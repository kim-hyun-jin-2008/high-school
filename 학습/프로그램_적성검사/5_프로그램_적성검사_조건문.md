...
### 설명 영상 - 3번 시청
https://www.youtube.com/watch?v=yihJQt6Y4dU

---

### 📘 조건문의 종류 비교 표 (JavaScript 기준)

| 이름          | 실제 문법 형태                       | 의미 / 역할                      | 언제 사용?            | 비고                      |
| ----------- | ------------------------------ | ---------------------------- | ----------------- | ----------------------- |
| **if**      | `if (조건) { }`                  | 조건이 참일 때만 실행                 | 가장 먼저 조건을 검사할 때   | 단독으로 사용 가능              |
| **if-else** | `if (조건) { } else { }`         | 조건이 참 → if 실행<br> 거짓 → else 실행 | 둘 중 하나는 반드시 실행될 때 | else는 조건 없음             |
| **else if** | `if (조건) { } else if (조건) { }` | 여러 조건을 순차적으로 검사              | 조건이 2개 이상일 때      | 반드시 **if 다음에만** 올 수 있음  |
| **else**    | `else { }`                     | if를 사용한 다음<br>위 조건들이 모두 거짓일 때 실행           | 마지막 기본 처리         | 맨 끝에 1번만 사용 가능<br>단독으로 쓰이는것 아님.          |
